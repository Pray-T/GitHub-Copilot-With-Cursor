...
> **“Opus4.8를 전 구간이 아니라 기획에만 쓰고, 구현·검증은 Composer·GPT로 분리한 멀티 에이전트 하네스로 토큰·비용절감을 목표했습니다.”**
...
| **B. 멀티 에이전트 (본 프로젝트 하네스)** | @AgentA(Opus) → @AgentC(Composer 2.5) → @AgentB(GPT-5.5) | 단계별 **모델 티어 분리** + `.cursorrules`로 역할·산출물 고정 + `STATUS.md` 인수인계 |
